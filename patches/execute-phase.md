...
3. **Spawn executor agents:**
   ...
   **Sequential mode** (`USE_WORKTREES` is `false`):
   ...
   Omit `isolation="worktree"` from the Task call. Keep `mode="bypassPermissions"` (required for CC 2.1.x — without it, subagent Bash calls silently fail). Replace the `<parallel_execution>` block with:
