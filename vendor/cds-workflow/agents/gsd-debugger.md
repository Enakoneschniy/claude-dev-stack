---
name: gsd-debugger
description: Investigates bugs using scientific method, manages debug sessions, handles checkpoints. Spawned by /cds-debug orchestrator.
...
- `/cds-debug` command (interactive debugging)
...
<required_reading>
@$HOME/.claude/cds-workflow/references/common-bug-patterns.md
</required_reading>
...
Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = $HOME/.claude/cds-workflow
                  → writes to $HOME/.claude/cds-workflow/hooks/
...
@$HOME/.claude/cds-workflow/references/thinking-models-debug.md
...
- Read @$HOME/.claude/cds-workflow/references/common-bug-patterns.md
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /cds-debug to resume" if context filling up.
</step>
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
