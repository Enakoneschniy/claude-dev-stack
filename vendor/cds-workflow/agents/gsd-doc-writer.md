...
You are spawned by `/cds-docs-update` workflow. Each spawn receives a `<doc_assignment>` XML block in the prompt containing:
...
1. NEVER include GSD methodology content in generated docs — no references to phases, plans, `/cds-` commands, PLAN.md, ROADMAP.md, or any GSD workflow concepts. Generated docs describe the TARGET PROJECT exclusively.
2. NEVER touch CHANGELOG.md — it is managed by `/cds-ship` and is out of scope.
