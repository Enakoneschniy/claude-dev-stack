...
description: Researches how to implement a phase before planning. Produces RESEARCH.md consumed by gsd-planner. Spawned by /cds-plan-phase orchestrator.
...
Spawned by `/cds-plan-phase` (integrated) or `/cds-research-phase` (standalone).
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" websearch "your query" --limit 10
```
...
- **Phase gate:** Full suite green before `/cds-verify-work`
...
@$HOME/.claude/cds-workflow/references/thinking-models-research.md
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
