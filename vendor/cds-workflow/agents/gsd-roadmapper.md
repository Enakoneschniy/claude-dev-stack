---
name: gsd-roadmapper
description: Creates project roadmaps with phase breakdown, requirement mapping, success criteria derivation, and coverage validation. Spawned by /cds-new-project orchestrator.
...
- `/cds-new-project` orchestrator (unified project initialization)
...
<downstream_consumer>
Your ROADMAP.md is consumed by `/cds-plan-phase` which uses it to:
...
This annotation is consumed by downstream workflows (`new-project`, `progress`) to suggest `/cds-ui-phase` at the right time. Phases without UI indicators omit the annotation entirely.
...
Reference full template: `$HOME/.claude/cds-workflow/templates/roadmap.md`

## STATE.md Structure

Use template from `$HOME/.claude/cds-workflow/templates/state.md`.
