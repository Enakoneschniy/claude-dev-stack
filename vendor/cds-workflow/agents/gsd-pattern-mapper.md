...
description: Analyzes codebase for existing patterns and produces PATTERNS.md mapping new files to closest analogs. Read-only codebase analysis spawned by /cds-plan-phase orchestrator before planning.
...
Spawned by `/cds-plan-phase` orchestrator (between research and planning steps).
