...
Spawned by `/cds-execute-phase` orchestrator.
...
@$HOME/.claude/cds-workflow/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@$HOME/.claude/cds-workflow/references/ios-scaffold.md
...
@$HOME/.claude/cds-workflow/references/executor-examples.md
</deviation_rules>
...
**See @$HOME/.claude/cds-workflow/references/checkpoints.md**
...
```bash
node $HOME/.claude/cds-workflow/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @$HOME/.claude/cds-workflow/templates/summary.md
...
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}"
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>
