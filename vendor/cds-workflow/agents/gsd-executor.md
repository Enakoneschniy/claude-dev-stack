...
```bash
node $HOME/.claude/cds-workflow/bin/cds-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}"
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>
