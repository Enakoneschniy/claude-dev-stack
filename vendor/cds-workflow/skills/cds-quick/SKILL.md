...
<execution_context>
@$HOME/.claude/cds-workflow/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" frontmatter get .planning/quick/{dir}/SUMMARY.md --field status 2>/dev/null
  ```
...
─────────────────────────────────────
Resume with: /cds-quick resume {slug}
```
...
5. Load context via:
   ```bash
   node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" init quick
   ```
...
Execute the quick workflow from @$HOME/.claude/cds-workflow/workflows/quick.md end-to-end.
