...
- File reference: `/cds-new-project --auto @prd.md`
...
Usage:
  /cds-new-project --auto @your-idea.md
  /cds-new-project --auto [paste or write your idea here]
...
```bash
INIT=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" init new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" agent-skills gsd-project-researcher 2>/dev/null)
AGENT_SKILLS_SYNTHESIZER=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" agent-skills gsd-synthesizer 2>/dev/null)
AGENT_SKILLS_ROADMAPPER=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" agent-skills gsd-roadmapper 2>/dev/null)
```
...
**If `project_exists` is true:** Error — project already initialized. Use `/cds-progress`.
...
- options:
  - "Map codebase first" — Run /cds-map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**

```
Run `/cds-map-codebase` first, then return to `/cds-new-project`
```
...
```bash
mkdir -p .planning
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "chore: add project config" --files .planning/config.json
```
...
**After each phase transition** (via `/cds-transition`):
...
**After each milestone** (via `/cds-complete-milestone`):
...
```bash
mkdir -p .planning
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "docs: initialize project" --files .planning/PROJECT.md
```
...
**Note:** Run `/cds-settings` anytime to update model profile, workflow agents, branching strategy, and other preferences.
...
Use template: $HOME/.claude/cds-workflow/templates/research-project/STACK.md
</output>
...
Use template: $HOME/.claude/cds-workflow/templates/research-project/FEATURES.md
</output>
...
Use template: $HOME/.claude/cds-workflow/templates/research-project/ARCHITECTURE.md
</output>
...
Use template: $HOME/.claude/cds-workflow/templates/research-project/PITFALLS.md
</output>
...
Use template: $HOME/.claude/cds-workflow/templates/research-project/SUMMARY.md
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" generate-claude-md --output "$INSTRUCTION_FILE"
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
Exit skill and invoke SlashCommand("/cds-discuss-phase 1 --auto")
...
```bash
PHASE1_SECTION=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" roadmap get-phase 1 2>/dev/null)
...
/cds-discuss-phase 1 — gather context and clarify approach

---

**Also available:**
- /cds-ui-phase 1 — generate UI design contract (recommended for frontend phases)
- /cds-plan-phase 1 — skip discussion, plan directly
...
- [ ] User knows next step is `/cds-discuss-phase 1`
