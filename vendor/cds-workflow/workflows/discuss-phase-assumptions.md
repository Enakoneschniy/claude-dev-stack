...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" agent-skills gsd-assumptions-analyzer 2>/dev/null)
```
...
Use /cds-progress to see available phases.
```
Exit workflow.
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
`/cds-plan-phase ${PHASE}`

---

**Also available:**
- `/cds-plan-phase ${PHASE} --skip-research` — plan without research
- `/cds-ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
Launch: `Skill(skill="cds-plan-phase", args="${PHASE} --auto")`
