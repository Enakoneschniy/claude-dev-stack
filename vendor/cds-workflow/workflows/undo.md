...
```
Usage: /cds-undo --last N | --phase NN | --plan NN-MM
...
```
Working tree has uncommitted changes. Commit or stash them before running /cds-undo.
```
...
**Also available:**
- `/cds-execute-phase ${PHASE}` — re-execute if needed
- `/cds-undo --last 1` — undo the revert itself if something went wrong
