...
- Check for MILESTONE-CONTEXT.md (from /cds-discuss-milestone)
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" phases clear --confirm
```

```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" init new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/cds-new-milestone --reset-phase-numbers ${GSD_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/cds-plan-phase` behavior. To change the default, use `/cds-settings`.
...
Use template: $HOME/.claude/cds-workflow/templates/research-project/{FILE}
</output>
...
**Commit requirements:**
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
`/cds-discuss-phase [N] ${GSD_WS}` — gather context and clarify approach

Also: `/cds-plan-phase [N] ${GSD_WS}` — skip discussion, plan directly
```
...
- [ ] User knows next step: `/cds-discuss-phase [N] ${GSD_WS}`
