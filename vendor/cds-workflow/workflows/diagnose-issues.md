...
```bash
AGENT_SKILLS_DEBUGGER=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" agent-skills gsd-debugger 2>/dev/null)
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
