...
@$HOME/.claude/cds-workflow/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" init execute-phase "${PHASE}")
...
INLINE_THRESHOLD=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" config-get workflow.inline_plan_threshold --default 2 2>/dev/null || echo "2")
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" phases list --type summaries --raw
...
See `$HOME/.claude/cds-workflow/references/tdd.md` for structure.
</tdd_plan_execution>
...
See $HOME/.claude/cds-workflow/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/cds-workflow/bin/cds-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/cds-workflow/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `$HOME/.claude/cds-workflow/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `$HOME/.claude/cds-workflow/templates/summary.md`.
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" state advance-plan

  # Recalculate progress bar from disk state
  node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" state update-progress

  # Record execution metrics
  node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" state record-metric \
...
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
if [ "$IS_WORKTREE" != "true" ]; then
  node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" roadmap update-plan-progress "${PHASE}"
fi
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/cds-execute-phase {phase}` + `/cds-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/cds-plan-phase {Z+1}` + `/cds-verify-work {Z}` + `/cds-discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/cds-complete-milestone` + `/cds-verify-work` + `/cds-add-phase` |
