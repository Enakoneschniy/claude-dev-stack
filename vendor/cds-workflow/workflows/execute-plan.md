...
```bash
INIT=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
INLINE_THRESHOLD=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" config-get workflow.inline_plan_threshold --default 2 2>/dev/null || echo "2")
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" phases list --type summaries --raw
...
```bash
NODE_REPAIR=$(node "./.claude/cds-workflow/bin/gsd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" state advance-plan

  # Recalculate progress bar from disk state
  node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" state update-progress

  # Record execution metrics
  node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" state record-metric \
...
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
if [ "$IS_WORKTREE" != "true" ]; then
  node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
fi
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
