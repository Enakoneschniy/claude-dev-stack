...
Seeds auto-surface during /cds-new-milestone when trigger conditions match the
...
This seed should be presented during `/cds-new-milestone` when the milestone
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
This seed will surface automatically when you run /cds-new-milestone
