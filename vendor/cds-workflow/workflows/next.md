...
```
No GSD project detected. Run `/cds-new-project` to get started.
```
Exit.
</step>
...
Run `/cds-health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
**Source phase:** {src}
**Deferred at:** {date} during /cds-next advancement to Phase {dest}
**Plans:**
...
→ Next action: `/cds-discuss-phase <first-phase>`
...
▶ **Next step:** `/cds-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/cds-next` is zero-friction advancement.
</step>
