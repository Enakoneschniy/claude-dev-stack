...
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" agent-skills gsd-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/cds-validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/cds-validate-phase {N}` |

Phases needing validation: run `/cds-validate-phase {N}` for each flagged phase.
...
- /cds-complete-milestone {version} — proceed anyway (accept tech debt)
