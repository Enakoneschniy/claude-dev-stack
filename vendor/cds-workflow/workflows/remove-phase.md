...
- Example: `/cds-remove-phase 17` → phase = 17
- Example: `/cds-remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /cds-remove-phase <phase-number>
Example: /cds-remove-phase 17
```
...
```bash
INIT=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" init phase-op "${target}")
...
To abandon current work, use /cds-pause-work instead.
```
...
```bash
RESULT=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
Would you like to:
- `/cds-progress` — see updated roadmap status
