...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" validate health $REPAIR_FLAG
```
...
- [E001] config.json: JSON parse error at line 5
  Fix: Run /cds-health --repair to reset to defaults

- [E002] PROJECT.md not found
  Fix: Run /cds-new-project to create
```
...
```
---
N issues can be auto-repaired. Run: /cds-health --repair
```
</step>
...
```
Would you like to run /cds-health --repair to fix N issues automatically?
```
...
```bash
node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" validate health
```
