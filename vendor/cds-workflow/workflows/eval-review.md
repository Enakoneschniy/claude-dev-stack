...
Use after /cds-execute-phase to verify that the evaluation strategy from AI-SPEC.md was actually implemented. Mirrors the pattern of /cds-ui-review and /cds-validate-phase.
</purpose>
...
**State C** — No SUMMARY.md: Exit — "Phase {N} not executed. Run /cds-execute-phase {N} first."
...
Consider running /cds-ai-integration-phase {N} before implementation next time.
```
...
```markdown
Read $HOME/.claude/agents/cds-eval-auditor.md for instructions.
...
{If PRODUCTION READY:}
  Next step: /cds-plan-phase (next phase) or deploy

{If NEEDS WORK:}
  Address critical gaps in EVAL-REVIEW.md, then re-run /cds-eval-review {N}
