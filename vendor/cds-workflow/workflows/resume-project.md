...
<required_reading>
@$HOME/.claude/cds-workflow/references/continuation-format.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" init resume)
...
**If `planning_exists` is false:** This is a new project - route to /cds-new-project
</step>
...
- This is the primary resumption source — structured data from `/cds-pause-work`
...
📋 [N] pending todos — /cds-check-todos to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/cds-execute-phase {phase} ${GSD_WS})
   OR
1. Discuss Phase 3 context (/cds-discuss-phase 3 ${GSD_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/cds-plan-phase 3 ${GSD_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show command for user to run after clearing:
  ...
  **Also available:**
  - `/cds-discuss-phase [N] ${GSD_WS}` — gather context first
  - `/cds-research-phase [N] ${GSD_WS}` — investigate unknowns
