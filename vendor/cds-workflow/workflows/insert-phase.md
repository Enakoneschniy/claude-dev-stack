...
```bash
INIT=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.claude/cds-workflow/bin/cds-tools.cjs" phase insert "${after_phase}" "${description}")
```
