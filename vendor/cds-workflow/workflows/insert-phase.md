...
Example: `/cds-insert-phase 72 Fix critical auth bug`
-> after = 72
...
Usage: /cds-insert-phase <after> <description>
Example: /cds-insert-phase 72 Fix critical auth bug
```
...
```bash
INIT=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "$HOME/.claude/cds-workflow/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
...
- Don't use this for planned work at end of milestone (use /cds-add-phase)
- Don't insert before Phase 1 (decimal 0.1 makes no sense)
- Don't renumber existing phases
- Don't modify the target phase content
- Don't create plans yet (that's /cds-plan-phase)
