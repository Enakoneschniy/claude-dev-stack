...
async function buildFilePathMap(vaultRoot) {
  ...
  for (const f of files) {
    ...
    // FIX-01: ADR files use projectScoped titles (no slug__ prefix), so titleIdx above is -1
    // and the second key is never added. But parseSourceTitle extracts localTitle as
    // 'ADR-NNNN-slug.md' from shared-notebook titles like 'slug__ADR-NNNN-slug.md'.
    // The disk basename is 'NNNN-slug.md', so the lookup at slug/ADR-NNNN-slug.md misses.
    // Fix: add an ADR-prefixed key so migration lookups resolve correctly.
    if (f.category === 'adr') {
      map.set(`${f.projectSlug}/ADR-${f.basename}`, f.absPath);
    }
  }
  return map;
}
