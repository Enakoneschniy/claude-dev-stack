...
import { existsSync, readdirSync, readFileSync, writeFileSync, cpSync, chmodSync, unlinkSync } from 'fs';
...
function updateHooks() {
  ...
  const hookFiles = readdirSync(hooksSrcDir).filter(f => f.endsWith('.sh') || f === 'cds-statusline.mjs' || f === 'cds-check-update.mjs');
  ...
  // Statusline hook (CDS replaces GSD)
  const statuslineDest = join(hooksDestDir, 'cds-statusline.mjs');
  if (existsSync(statuslineDest)) {
    if (!settings.statusLine || !settings.statusLine.command?.includes('cds-statusline')) {
      settings.statusLine = {
        type: 'command',
        command: `node "${statuslineDest}"`,
      };
      ok('Statusline hook registered (CDS)');
      settingsChanged = true;
    }
  }
...
export async function main() {
  ...
  const versionBefore = pkgJson.version;
  ...
  if (hasGsd) additional.push('CDS workflow engine');
  ...
  if (vaultPath) {
    ...
    // Refresh project-level skills and CLAUDE.md in mapped projects
    const { setupAllProjects } = await import('./project-setup.mjs');
    const { projects: projectCount, results, missing } = setupAllProjects(vaultPath);
    if (projectCount > 0) {
      const changed = results.filter(r => r.success && (r.skillsCopied > 0 || r.claudeMd !== 'unchanged'));
      if (changed.length > 0) {
        ok(`Refreshed skills + CLAUDE.md in ${changed.length}/${projectCount} project(s)`);
      } else {
        info('Project-level skills already up to date');
      }
    }
    if (missing && missing.length > 0) {
      warn(`${missing.length} mapped project(s) missing on disk:`);
      for (const m of missing) {
        console.log(`    ${c.dim}-${c.reset} ${m.project} ${c.dim}(${m.path})${c.reset}`);
      }
      info(`Run: claude-dev-stack projects remove <name>  to clean up stale entries`);
    }
  ...
  // Re-read version after update (package may have been replaced)
  let versionAfter = versionBefore;
  try {
    const updatedPkg = JSON.parse(readFileSync(join(PKG_ROOT, 'package.json'), 'utf8'));
    versionAfter = updatedPkg.version;
  } catch {}

  console.log('');
  if (versionAfter !== versionBefore) {
    ok(`${c.bold}Update complete${c.reset}  ${c.dim}${versionBefore}${c.reset} → ${c.green}${versionAfter}${c.reset}`);
  } else {
    ok(`${c.bold}Update complete${c.reset}  ${c.dim}v${versionBefore} (up to date)${c.reset}`);
  }

  // Clear update-check cache so statusline reflects new state
  const cachePath = join(homedir(), '.cds', 'update-check.json');
  try {
