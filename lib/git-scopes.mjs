...
/**
 * Build a GitHub Actions commitlint workflow YAML string.
 *
 * @param {object} config  must have main_branch
 * @returns {string} YAML content
 */
export function buildCommitlintYml(config) {
  const mainBranch = (config && config.main_branch) || 'main';
  const lines = [
    'name: Commitlint',
    '',
    'on:',
...
/**
 * Parse a CLAUDE.md string and extract git convention data.
 ...
 */
export function parseClauda(content) {
  const empty = { scopes: [], types: [], mainBranch: null, ticketPrefix: null };
  ...
  // P2: bullet list under heading matching scopes|packages|modules|services|components
  const headingRegex = /^#{1,4}\s+(scopes|packages|modules|services|components)\s*$/im;
  ...
  if (headingMatch) {
    ...
    for (const line of lines) {
      ...
      if (m) rawScopes.push(m[1]);
    }
  }

  // P3: inline "scopes: word1, word2"
  if (rawScopes.length === 0) {
    const inlineMatch = /\bscopes?\s*:\s*([^\n]+)/i.exec(content);
    if (inlineMatch) {
      rawScopes = inlineMatch[1].split(/[,\s]+/).filter(s => /^\w[\w-]*$/.test(s));
    }
  }

  // P4: backtick-quoted identifiers appearing 2+ times
  if (rawScopes.length === 0) {
    const counts = {};
    const btRegex = /`(\w[\w-]{1,20})`/g;
    let m;
    while ((m = btRegex.exec(content)) !== null) {
      counts[m[1]] = (counts[m[1]] || 0) + 1;
    }
    rawScopes = Object.keys(counts).filter(k => counts[k] >= 2);
  }

  // Post-process: deduplicate, lowercase, hyphenate spaces, max 30
  const scopes = [...new Set(rawScopes.map(s => s.toLowerCase().replace(/\s+/g, '-')))].slice(0, 30);

  // ── Types ──
  const knownTypes = ['feat', 'fix', 'refactor', 'test', 'docs', 'ci', 'chore'];
  const types = knownTypes.filter(t => new RegExp('\\b' + t + '\\b').test(content));

  // ── Main branch ──
  let mainBranch = null;
  const mbMatch1 = /\bmain\s+branch\s*:?\s*([\w/-]+)/i.exec(content);
  if (mbMatch1) {
    mainBranch = mbMatch1[1];
  } else {
    const mbMatch2 = /\bbranch\s*[:\s]+\b(main|master|develop|\w+)\b/i.exec(content);
    if (mbMatch2) mainBranch = mbMatch2[1];
  }

  // ── Ticket prefix ──
  let ticketPrefix = null;
  const tpMatch1 = /\bticket\s+prefix\s*:?\s*([A-Z]{2,6}-)/i.exec(content);
  if (tpMatch1) {
    ticketPrefix = tpMatch1[1];
  } else {
    const tpMatch2 = /\b([A-Z]{2,6})-\d+\b[\s\S]{0,200}\b\1-\d+\b/.exec(content);
    if (tpMatch2) {
      ticketPrefix = tpMatch2[1] + '-';
    }
  }

  return { scopes, types, mainBranch, ticketPrefix };
}
