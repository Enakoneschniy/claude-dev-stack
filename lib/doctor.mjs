...
/**
 * Detect whether a Notion MCP server is configured in the Claude CLI.
 * Uses `claude mcp list` (plaintext) and checks for a line containing "notion".
 *
 * Output format: "claude.ai Notion: https://... - ✓ Connected"
 *
 * Returns { found: boolean, error?: 'exec_failed' }
 * Caller must ensure `claude` CLI is available before calling.
 *
 * @returns {{ found: boolean, error?: string }}
 */
export function checkNotionMcp() {
  try {
    const raw = execFileSync('claude', ['mcp', 'list'], {
      encoding: 'utf8',
      timeout: 15000,
    });
    const found = raw.toLowerCase().includes('notion');
    return { found };
  } catch {
    return { found: false, error: 'exec_failed' };
  }
}

export async function main(opts = {}) {
  // --gsd-permissions: write GSD executor Bash patterns to .claude/settings.local.json
  if (opts.gsdPermissions) {
    ...
    try {
      ...
      if (result.added.length > 0) {
        ok(`Added ${result.added.length} GSD permission pattern(s) to .claude/settings.local.json`);
        for (const p of result.added) info(`  + ${p}`);
      } else {
        ok('All GSD permission patterns already present — no changes needed');
      }
    } catch (err) {
      fail(`Could not write GSD permissions: ${err.message}`);
      info('  Check that .claude/ directory is writable in this project');
    }
    return;
  }
  ...
  if (hasNotebooklm) {
    ...
    // Per-project notebook stats (D-08: NBLM-V2-09)
    if (vaultPath) {
      try {
        ...
        if (cdsNotebooks.length === 0) {
          ...
          for (const nb of cdsNotebooks) {
            ...
            try {
              ...
              totalSources += count;
              breakdown.push(`${slug}: ${count} source${count === 1 ? '' : 's'}`);
            } catch {
              breakdown.push(`${slug}: ? (error)`);
            }
          }
          const nbCount = cdsNotebooks.length;
          ok(`${nbCount} notebook${nbCount === 1 ? '' : 's'}, ${totalSources} source${totalSources === 1 ? '' : 's'} total`);
          for (const line of breakdown) {
            info(`  ${line}`);
          }
        }
      } catch {
        // listNotebooks failed — don't block doctor
        info('per-project stats: unavailable');
      }
    }

    // NOTEBOOKLM_NOTEBOOK_NAME deprecation (D-09: NBLM-V2-08)
  ...
  if (!hasCommand('claude')) {
    info('claude CLI not found — skipping Notion MCP check');
  } else {
    const notionCheck = checkNotionMcp();
    if (notionCheck.error === 'exec_failed') {
      warn('Could not check Notion MCP status');
      warnings++;
    } else if (notionCheck.found) {
      ok('Notion MCP server — configured');
    } else {
      fail('Notion MCP server — not configured');
      info('Install via: claude mcp add notion');
      issues++;
    }
  }
