 ...
 * Uses `claude mcp list --json` and checks for an entry whose name contains "notion".
 *
...
export function checkNotionMcp() {
  try {
    const raw = execFileSync('claude', ['mcp', 'list', '--json'], {
      encoding: 'utf8',
      timeout: 10000,
    });
    // Mitigate T-09-08: JSON.parse in try/catch; malformed output triggers warn, not crash.
    let servers;
    try {
    ...
    // Accept both array and object-of-servers formats
    const entries = Array.isArray(servers) ? servers : Object.values(servers);
    const found = entries.some(entry => {
      const name = (entry.name || entry.command || '').toLowerCase();
      return name.includes('notion');
    });
    return { found };
  } catch {
    return { found: false, error: 'exec_failed' };
