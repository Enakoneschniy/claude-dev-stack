...
export async function syncVault(opts = {}) {
  ...
  // Legacy single-notebook mode: build a flat scoped sub-object merging all projects.
  // Task 2 will replace this with per-project loop + per-project scoped sub-objects.
  // Merge all existing project file entries into a single flat files map.
  const legacyFiles = {};
  for (const proj of Object.values(manifest.projects ?? {})) {
    Object.assign(legacyFiles, proj.files ?? {});
  }
  const scopedManifest = { files: legacyFiles };

  // Main loop — per-file sync with rate-limit abort.
  for (const fileEntry of files) {
    try {
      await syncOneFile({ fileEntry, vaultRoot, notebookId, manifest: scopedManifest, stats, dryRun });
    } catch (err) {
      if (err instanceof NotebooklmRateLimitError) {
        // D-08: rate limit aborts the sync run.
        stats.rateLimited = true;
        break;
      }
  ...
  // Write updated manifest back to disk. Distribute flat files into per-project buckets.
  if (!dryRun && (stats.uploaded > 0 || stats.failed > 0)) {
    for (const [filePath, entry] of Object.entries(scopedManifest.files)) {
      const parts = filePath.split('/');
      const slug = (parts[0] === 'projects' && parts.length >= 3) ? parts[1] : '_shared';
      if (!manifest.projects[slug]) {
        manifest.projects[slug] = { notebook_id: notebookId, files: {} };
      }
      manifest.projects[slug].files[filePath] = entry;
    }
