...
export async function main(args = []) {
  ...
  switch (subcommand) {
    ...
    default:
      ...
      console.log(`    ${c.white}claude-dev-stack projects enable${c.reset}    ${c.dim}Copy skills + update CLAUDE.md in mapped projects${c.reset}`);
      console.log('');
  }
}

// ── Enable project-level skills ─────────────────────────────────
async function enableProjectSkills() {
  console.log('');
  console.log(`  ${c.bold}Enable project-level skills${c.reset}`);
  console.log(`  ${c.dim}Copies skills to {project}/.claude/skills/ and updates CLAUDE.md.${c.reset}`);
  console.log(`  ${c.dim}Required for auto-invocation via Skill tool.${c.reset}`);
  console.log('');
  ...
  const mapPath = join(vaultPath, 'project-map.json');
  if (!existsSync(mapPath)) {
    warn('No project-map.json found');
    info('Run: claude-dev-stack projects map');
    return;
  }

  const { setupAllProjects } = await import('./project-setup.mjs');
  const { projects: count, results, missing } = setupAllProjects(vaultPath);

  if (count === 0 && (!missing || missing.length === 0)) {
    warn('No projects to set up');
    return;
  }

  for (const r of results) {
    if (r.success) {
      ok(`${r.project}: ${r.skillsCopied} skills, CLAUDE.md ${r.claudeMd}`);
    } else {
      fail(`${r.project}: ${r.error}`);
    }
  }
