...
export async function mapProjects() {
  ...
  console.log(`    ${c.dim}For each project, specify the source code directory.${c.reset}`);
  console.log(`    ${c.dim}Press Enter to skip, Tab to autocomplete.${c.reset}`);
  console.log('');

  for (const p of projects) {
    const currentPath = reverseMap[p.name];
    const hint = currentPath
      ? `${c.green}✔${c.reset} ${c.bold}${p.name}${c.reset} → ${c.dim}${currentPath.replace(homedir(), '~')}${c.reset}`
      : `${c.yellow}?${c.reset} ${c.bold}${p.name}${c.reset} ${c.dim}(no path mapped)${c.reset}`;
    ...
    const { change } = await prompt({
      type: 'confirm',
      name: 'change',
      message: currentPath ? 'Change path?' : 'Set path?',
      initial: !currentPath,
    });

    if (change) {
      const path = await askPathAutocomplete(`Path to ${p.name}`, currentPath || '');
      if (path) {
        const resolved = path.replace(/^~/, homedir()).replace(/\/+$/, '');
        if (existsSync(resolved)) {
          updateProjectMap(vaultPath, resolved, p.name);
          ok(`${p.name} → ${resolved.replace(homedir(), '~')}`);
        } else {
