...
// ── Binary artifact type set ─────────────────────────────────────────────────

/**
 * Artifact types that produce binary output (audio, video, etc.).
 * For these types, generateArtifact returns content: null and skips the
 * download step. Text types (report, quiz, flashcards, etc.) trigger a
 * post-generation download step to retrieve content from the output file.
 *
 * Exported for use by the CLI layer (Plan 02) to determine download behavior.
 */
...
/**
 * Generate an artifact from a NotebookLM notebook and return its content.
 *
 * Uses `notebooklm generate {type} --wait --retry 2 --json` to generate and
 * block until ready. For text artifact types, a second
 * `notebooklm download {type} --json` call retrieves the content by reading
 * from the file path returned in `output_path`. For binary types (audio,
 * video, etc.) content is returned as null — the caller is responsible for
 * any further download.
 *
 * Retry is delegated to notebooklm-py via `--retry 2`. No wrapper-level retry.
 *
 * @param {string} notebookId - ID of the target notebook
 * @param {string} type       - Artifact type (report, quiz, audio, video, etc.)
 * @param {object} [options]
 * @param {Function} [options._runFn] - Injectable run function for testing (replaces runNotebooklm)
 * @returns {Promise<{artifactId: string, content: string|null, type: string}>}
 *   content is the text content for text types, null for binary types
 *
 * @throws {TypeError}                   if notebookId or type are missing/invalid
 * @throws {NotebooklmNotInstalledError} if notebooklm binary is missing from PATH
 * @throws {NotebooklmCliError}          if generation does not complete or any other failure
 */
export async function generateArtifact(notebookId, type, options = {}) {
  if (typeof notebookId !== 'string' || notebookId.length === 0) {
    throw new TypeError('generateArtifact: notebookId must be a non-empty string');
  }
  if (typeof type !== 'string' || type.length === 0) {
    throw new TypeError('generateArtifact: type must be a non-empty string');
  }

  // Allow test injection of the run function to avoid real CLI invocations.
  const run = options._runFn || ((args, opts) => runNotebooklm(args, opts));

  // Step 1: Generate and wait for completion (delegates retry to notebooklm-py via --retry 2).
  ...
  // Step 2: For text types, download and read content from the output_path file.
  // Binary types return content: null immediately (caller handles download separately).
  let content = null;
  if (!BINARY_ARTIFACT_TYPES.has(type)) {
    ...
    // Content is in the FILE at output_path — NOT inline in the JSON (research-verified).
    // WR-02: guard against dlResult missing output_path or pointing to a non-existent file.
