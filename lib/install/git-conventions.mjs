...
export async function installGitConventions(projectsData, stepNum, totalSteps) {
  ...
  for (const project of projects) {
    ...
    const { acceptScopes } = await prompt([{
      ...
      choices: [
        { title: 'Accept detected scopes', value: 'accept' },
        { title: 'Skip (configure manually later)', value: 'skip' },
      ],
      initial: 0,
    }]);
    // If user rejects scopes, they can run `claude-dev-stack scopes init` later
    if (acceptScopes === 'skip') {
    ...
    const { acceptBranch } = await prompt([{
      ...
      choices: [
        ...
        { title: 'Change branch name', value: 'change' },
      ],
      initial: 0,
    }]);
    if (acceptBranch === 'change') {
    ...
    if (existsSync(join(dirPath, 'package.json'))) {
      const { wantCommitlint } = await prompt([{
        ...
        choices: [
          { title: 'Install (print instructions)', value: 'install' },
          { title: 'Skip', value: 'skip' },
        ],
        initial: 1,
      }]);
      if (wantCommitlint === 'install') {
