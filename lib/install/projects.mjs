...
import { c, ok, warn, info, prompt, askPath, step, existsSync, listDirs, homedir } from '../shared.mjs';
import { toSlug } from '../project-naming.mjs';

// ── Read registered project paths from project-map.json ────────
// Returns Map<path, name> for DX-09 name lookup
...
export async function collectProjects(totalSteps, detectedProjects, detectedBaseDir, vaultPath) {
  ...
  const { hasBaseDir } = await prompt({
    ...
    choices: [
      { title: 'Yes, scan a parent directory', value: 'base-dir' },
      { title: 'No, add paths manually', value: 'manual' },
    ],
    initial: 0,
  });
  ...
  if (hasBaseDir === 'base-dir') {
    ...
    const { addMore } = await prompt({
      ...
      choices: [
        { title: 'Yes, add more', value: 'yes' },
        { title: 'Done, continue', value: 'done' },
      ],
      initial: 1,
    });
