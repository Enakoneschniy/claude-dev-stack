...
// Returns a Map<path, name> for BUG-03 pre-selection and DX-09 name lookup
...
export async function collectProjects(totalSteps, detectedProjects, detectedBaseDir, vaultPath) {
  ...
  // BUG-03 / DX-09: Map<path, name> of already-registered projects from project-map.json
  const registeredPaths = _readRegisteredPaths(vaultPath);
  ...
  if (hasBaseDir) {
    ...
    // DX-08: pre-fill with detectedBaseDir derived from project-map.json
    ...
    if (dirs.length === 0) {
      ...
      const { selected } = await prompt({
        ...
        choices: dirs.map(d => ({
          title: d.name,
          value: d.path,
          // BUG-03: pre-check dirs already in project-map.json or detected projects
      ...
      // Ask project name for each selected directory (DX-09: skip already-registered)
      for (const dirPath of sel) {
        const dirName = basename(dirPath);
        const defaultName = toSlug(dirName);

        // DX-09: skip name prompt if this path is already in project-map.json
        if (registeredPaths.has(dirPath)) {
          const existingName = registeredPaths.get(dirPath) || defaultName;
          ok(`${existingName} → ${c.dim}${dirPath.replace(homedir(), '~')}${c.reset} ${c.green}(already registered)${c.reset}`);
