...
export async function selectAndInstallPlugins(stepNum, totalSteps, detectedUseCase) {
  ...
  if (!hasCommand('claude')) {
    ...
    return { installed: [], failed: [], useCase: detectedUseCase || null };
  }
  ...
  if (!hasSuperpowers) {
    ...
    if (result.status === 0) {
      ...
      info('superpowers — skipped (Claude CLI unavailable or network issue, optional)');
    }
    console.log('');
  } else {
  ...
  // DX-10 / D-04: Pre-fill use case with select "Keep current / Change"
  let useCase;
  if (detectedUseCase) {
    const useCaseLabels = {
