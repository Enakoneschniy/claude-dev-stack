...
export function installObsidianSkills(skillsDir, stepNum, totalSteps, pkgRoot) {
  ...
  if (result.status === 0) {
    ...
    info('Obsidian Skills — skipped (GitHub unavailable, optional)');
    return false;
  }
}
...
export function installCustomSkills(skillsDir, stepNum, totalSteps, pkgRoot) {
  ...
  const skillNames = ['session-manager', 'dev-research', 'cds-quick', 'budget-continue', 'notion-importer'];
...
export function installDeepResearch(skillsDir, agentsDir, stepNum, totalSteps) {
  ...
  if (result.status !== 0) {
    info('Deep Research — skipped (GitHub unavailable, optional)');
    return false;
  }
