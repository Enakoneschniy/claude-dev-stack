...
// DX-12: alreadyAuthenticated is detected by detectInstallState() and passed in
export async function installNotebookLM(pipCmd, stepNum, totalSteps, alreadyAuthenticated) {
  ...
  // Step 3: DX-12 — skip login if already authenticated (detected via detectInstallState).
  if (alreadyAuthenticated) {
    // Verify auth is still valid
    const authStillValid = runCmd('notebooklm auth check 2>/dev/null') !== null;
    if (authStillValid) {
      ok('NotebookLM: authenticated (skipping login)');
    } else {
  ...
  if (!alreadyAuthenticated) {
    // Run `notebooklm login` interactively (D-10).
    ...
    // Verify with auth check (D-10 post-login).
  ...
  // Step 4 (DX-12): Use "Run sync now?" for re-installs, "Run first sync?" for new.
  const { runFirstSync } = await prompt({
    type: 'confirm',
    name: 'runFirstSync',
    message: alreadyAuthenticated ? 'Run sync now?' : 'Run first NotebookLM sync now? (recommended)',
    initial: true,
  });
  if (runFirstSync) {
