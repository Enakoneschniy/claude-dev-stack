...
// BUG-01/02: Writes hooks + allowedTools to each project's .claude/settings.json
...
export function installSessionHook(stepNum, totalSteps, pkgRoot, vaultPath, projectsData) {
  ...
  for (const name of ['session-start-context.sh', 'session-end-check.sh', 'vault-auto-push.sh', 'gsd-auto-reapply-patches.sh']) {
...
// ── Write hooks + allowedTools to a settings.json file ─────────
function _writeSettingsFile(settingsPath, startDest, endDest, pushDest, vaultPath) {
  ...
  // BUG-02: allowedTools — vault read/write + safe bash commands
  if (vaultPath) {
