...
export function installSessionHook(stepNum, totalSteps, pkgRoot, vaultPath, projectsData) {
  ...
  for (const name of ['session-start-context.sh', 'session-end-check.sh', 'vault-auto-push.sh', 'gsd-auto-reapply-patches.sh', 'budget-check.mjs', 'budget-reset.mjs']) {
...
function _writeSettingsFile(settingsPath, startDest, endDest, pushDest, vaultPath) {
  ...
  const budgetResetDest = join(homedir(), '.claude', 'hooks', 'budget-reset.mjs');
  ...
  const budgetCheckDest = join(homedir(), '.claude', 'hooks', 'budget-check.mjs');
