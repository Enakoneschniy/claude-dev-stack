# Milestone v0.8: NotebookLM Auto-Sync — ARCHIVED

**Status:** ✅ SHIPPED 2026-04-11
**Phases:** 1–5 (all complete)
**Total Plans:** 10 (2 + 2 + 1 + 2 + 3)
**Duration:** 2 days (2026-04-10 → 2026-04-11)
**Git range:** `bcd4333..a5ea12c` (84 commits)
**Test count:** 54 → 243 (+189, +350% growth)
**Dependencies:** `{"prompts": "^2.4.2"}` — single-dep constraint preserved
...
**Coverage:** 36/36 v1 requirements mapped and completed
...
**Plans**: 3 plans
Plans:
- [x] 05-01-PLAN.md — lib/notebooklm-cli.mjs dispatcher + bin/cli.mjs routing + help + TEST-02 smoke (NBLM-19, 20, 24, 25, TEST-02)
- [x] 05-02-PLAN.md — hooks/notebooklm-sync-trigger.mjs + runner + session-end-check.sh wiring (NBLM-21, 22, 23)
- [x] 05-03-PLAN.md — install wizard replacement + doctor section + Phase 3 gitignore migration (NBLM-26, 27)
**UI hint**: no
...
## Milestone Summary (appended on archive)

### Timeline
- **2026-04-10:** Milestone initialized, phases 1-3 context captured (3 parallel discusses)
- **2026-04-10:** Phase 2 discuss uncovered no-public-API blocker → **ADR-0001 pivot** to CLI wrapper
- **2026-04-10:** Phase 1 + Phase 2 shipped (opus executor, calibration phase)
- **2026-04-11:** Phase 3 shipped (sonnet executor, balanced profile active)
- **2026-04-11:** Phase 4 full cycle (discuss + research + plan + execute + verify)
- **2026-04-11:** 11 retroactive ADRs written (0002..0012)
- **2026-04-11:** Phase 5 full cycle, Task 4 automated-verified, milestone shipped

### Key Architectural Decisions (captured as ADRs in vault)
- **ADR-0001** NotebookLM integration via CLI wrapper (`notebooklm-py`, not REST) — mid-milestone pivot
- **ADR-0002** Session lifecycle hooks: Node wrapper + dual invocation pattern (Phase 1)
- **ADR-0003** Lazy binary detection with per-process cache for optional system deps (Phase 2)
- **ADR-0004** Retry delegation to upstream `--retry` flag (no in-process retry loops)
- **ADR-0005** Versioned manifest schema with implicit magic-number validation (Phase 3)
- **ADR-0006** Corrupt manifest recovery: rename + empty return, never write during read
- **ADR-0007** Vault-relative POSIX paths for cross-platform manifest stability
- **ADR-0008** Sessions are strictly upload-once (literal "append-only" enforcement) — largest UX decision
- **ADR-0009** `{project}__` naming convention with per-category filename rules
- **ADR-0010** Semantic drift policy — ROADMAP SC literal vs spirit governance
- **ADR-0011** Split trigger/runner architecture for detached background operations (Phase 5)
- **ADR-0012** Doctor severity discipline — `info` for optional-feature absence

### Issues Resolved
- **Session-manager context.md bug** — prose comments instead of executable code в SKILL.md, causing `/end` to silently not update context.md. Fixed in Phase 1 with Node wrapper + dual invocation.
- **`ensureManifestGitignored` idempotency sentinel bug** — pre-existing sentinel blocked extending managed block на existing Phase 3 vaults. Caught by Phase 5 research, fixed inline in 05-03.
- **`bin/install.mjs::installNotebookLM` wrong install path** — existing function used `pip install [browser] --break-system-packages` + manual-instructions-only UX. Replaced in Phase 5 with full D-08..D-11 wizard flow.
- **Name collision** — `case 'status':` at bin/cli.mjs:158 routes to analytics. Phase 5 `notebooklm status` correctly scoped as subcommand inside lib/notebooklm-cli.mjs — no collision.

### Issues Deferred (v2)
- Per-project NotebookLM notebooks (migration from shared) — NBLM-V2-02..03
- Notion MCP automation for docs sync — NBLM-V2-01
- Cron-based periodic sync — NBLM-V2-04
- Delta sync for context.md — NBLM-V2-05
- Conflict detection on manual notebook edits — NBLM-V2-06
- `shared/patterns.md` cross-project sync — NBLM-V2-07
- Analytics dashboard integration — NBLM-V2-09

### Technical Debt
- **STATE.md structural cleanup** — tool-managed frontmatter + freeform body drift during execution; requires manual reconciliation few times per milestone. Worth solving structurally in a tooling phase.
- **GSD branch naming drift** — template `gsd/v0.8-milestone` vs actual branch `gsd/v08-notebooklm-sync`; planner agent sometimes re-creates template branch, requiring consolidation merge. Pre-set `branching_strategy` + branch detection logic fix.
- **Background worktree spawn fragility** — parallel executor spawn in background with `isolation: "worktree"` hit edge cases (stale base, Bash permission denials during reset). Workaround: sequential inline execution. Worth fixing in harness.
- **Executor commit split anomaly** — final `docs(SUMMARY)` commit sometimes lands on main branch directly instead of worktree branch (observed в Phase 4 Wave 1 + Phase 5 Wave 1). Requires manual cherry-pick to consolidate. Not blocking but repeatable.
- **Vault decisions/ vs .planning/CONTEXT.md parallel systems** — reconciled retroactively in Phase 5 (12 ADRs written). Going forward, each discuss phase should produce both CONTEXT.md AND selective ADRs for architectural decisions. Could be automated as post-discuss-phase hook.
